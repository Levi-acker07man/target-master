      ...
      <div className="flex-1 p-8 lg:p-16 flex flex-col bg-[#faf9f6] overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4 shrink-0">
          <div>
            ...
            <p className="text-stone-500 font-medium text-lg">Saka Samvat: {indianDateString}</p>
          </div>
          <div className="bg-white px-6 py-3 rounded-2xl shadow-sm border border-stone-200 flex items-center gap-4">
             ...
             <span className="font-mono font-bold text-stone-700 tracking-wider text-lg">{timeString} IST</span>
          </div>
        </div>
        ...
        {!user ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center border-2 border-dashed border-stone-200 rounded-[3rem] p-12 bg-stone-50/50">
             ...
             <h2 className="text-3xl font-bold text-stone-800 mb-4">Welcome to Target Master</h2>
          ...
          <>
                 ...
                 {calendarDates.map((d, i) => {
                   ...
                   return (
                     <motion.div 
                       ...
                       className={`flex flex-col items-center justify-center min-w-[5.5rem] p-4 rounded-3xl cursor-pointer transition-all border ${
                     ...
                     >
                       <span className="text-[11px] font-bold uppercase tracking-widest opacity-80">{d.toLocaleDateString('en-IN', { weekday: 'short' })}</span>
                       <span className={`text-2xl font-black mt-1 ${isSelected ? 'text-white' : 'text-stone-800'}`}>{d.getDate()}</span>
                       <span className="text-xs font-medium opacity-80 mt-1">{d.toLocaleDateString('en-IN', { month: 'short' })}</span>
                     </motion.div>
                   )
                 })}
               </div>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 overflow-y-auto pb-10 pr-4 max-w-5xl">
                 <div className="bg-[#fff4c2] p-8 rounded-3xl shadow-sm border border-white h-fit">
                   ...
                   <div className="flex flex-col gap-4">
                     <input 
                       className="bg-white/60 p-4 rounded-2xl border-none focus:ring-2 focus:ring-yellow-400/50 outline-none text-stone-700 text-base font-medium"
                       value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && addTask()} placeholder="What needs to be done?"
                     />
                     <button onClick={addTask} className="bg-[#222222] text-white py-4 rounded-2xl font-bold hover:bg-stone-800 transition-colors shadow-sm">Pin to Timeline</button>
                   </div>
                 </div>

                 <div className="bg-[#dcf0f5] p-8 rounded-3xl shadow-sm border border-white flex flex-col min-h-[400px]">
                   <h3 className="text-xl font-bold mb-6 text-stone-800 flex justify-between items-center">
                   ...
                   <div className="flex-1 space-y-4 overflow-y-auto">
                     <AnimatePresence>
                       {visibleTasks.map(t => {
                         ...
                         return (
                           <motion.div key={t.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.9 }} className="flex items-start gap-4 bg-white/60 p-4 rounded-2xl border border-white shadow-sm group">
                             {isFuture ? (
                               <button onClick={() => deleteTask(t.id)} className="mt-1 text-rose-400 hover:text-rose-600 font-bold px-1 transition-colors">✕</button>
                             ) : (
                               <input type="checkbox" checked={t.completed} onChange={() => toggleTask(t.id, t.completed)} className="mt-1.5 w-5 h-5 rounded border-stone-400 cursor-pointer accent-[#222222]" />
                             )}
                             <span className={`text-base leading-relaxed transition-all flex-1 ${t.completed && !isFuture ? "line-through text-stone-400" : "text-stone-700 font-medium"}`}>{t.text}</span>
                             {!isFuture && (
                               <button onClick={() => deleteTask(t.id)} className="text-stone-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity text-lg">✕</button>
                             )}
                           </motion.div>
                         );
                       })}
                       {visibleTasks.length === 0 && <div className="text-stone-500 text-base italic mt-6 text-center">No targets pinned for this date.</div>}
                     </AnimatePresence>
                   </div>
                 </div>
               </div>
            ...
            : activeTab === "Today" ? (
              <div className="max-w-3xl w-full bg-[#dcf0f5] p-10 rounded-[3rem] shadow-sm border border-white flex flex-col h-[80vh]">
                   <div className="mb-10 bg-white/50 p-6 rounded-3xl border border-white shadow-sm">
                      <div className="flex justify-between items-end mb-3">
                        <h3 className="text-stone-800 font-bold text-lg">Today's Focus Level</h3>
                        <span className="text-4xl font-black text-[#222222]">{todayEfficiency}%</span>
                      </div>
                   ...
                   <h2 className="text-2xl font-bold mb-6 text-stone-800 flex justify-between items-center">
                     Action Items
                     <span className="text-sm font-bold bg-white/80 text-stone-600 px-4 py-1.5 rounded-xl shadow-sm">
                   ...
                   <div className="flex-1 overflow-y-auto space-y-4 pr-2">
                     <AnimatePresence>
                        {todayTasks.length === 0 ? (
                          ...
                          todayTasks.map(t => (
                            <motion.div key={t.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-4 bg-white/70 p-5 rounded-2xl border border-white shadow-sm group">
                              <input type="checkbox" checked={t.completed} onChange={() => toggleTask(t.id, t.completed)} className="mt-1 w-6 h-6 rounded border-stone-400 cursor-pointer accent-[#222222]" />
                              ...
                              <button onClick={() => deleteTask(t.id)} className="text-stone-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity text-2xl">✕</button>
                            </motion.div>
                          ))
                        )}
                     </AnimatePresence>
            ...
            : activeTab === "Upcoming" ? (
              <div className="max-w-4xl w-full overflow-y-auto pr-4 pb-10 space-y-8">
                {sortedUpcomingDates.length === 0 ? (
                  <div className="bg-white p-12 rounded-[3rem] border border-stone-200 text-center text-stone-500 italic text-lg">No upcoming targets on the calendar.</div>
                ) : (
                  sortedUpcomingDates.map(dateKey => (
                    <div key={dateKey} className="bg-[#fff4c2] p-8 rounded-[2rem] shadow-sm border border-white">
                      <h3 className="text-2xl font-bold mb-6 text-stone-800 border-b border-yellow-200/60 pb-3">
                        {new Date(dateKey).toLocaleDateString('en-IN', { weekday: 'long', month: 'long', day: 'numeric' })}
                      </h3>
                      <div className="space-y-4">
                        {upcomingTasksByDate[dateKey].map(t => (
                          <div key={t.id} className="flex items-start gap-4 bg-white/60 p-4 rounded-2xl border border-white shadow-sm group">
                            <button onClick={() => deleteTask(t.id)} className="mt-1 text-rose-400 hover:text-rose-600 font-bold px-2 text-lg">✕</button>
                            <span className="text-lg text-stone-700 font-medium flex-1">{t.text}</span>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            : activeTab === "Progress" ? (
              <div className="max-w-3xl w-full bg-white p-12 rounded-[3rem] shadow-sm border border-stone-200 flex flex-col h-[80vh] overflow-y-auto">
                 <h2 className="text-4xl font-bold mb-3 text-stone-800">Consistency Matrix</h2>
                 <p className="text-stone-500 text-lg mb-12 border-b border-stone-100 pb-6">Historical distribution of your daily task completion rates.</p>

                 {totalDaysWithTasks === 0 ? (
                    <div className="flex-1 flex items-center justify-center text-stone-400 italic bg-stone-50 rounded-3xl border-2 border-dashed border-stone-200 text-center px-8 text-lg">
                    ...
                    <div className="space-y-10 mt-4">
                      <div>
                        ...
                        <div className="w-full bg-stone-100 h-6 rounded-full overflow-hidden">
                      ...
                      <div className="mt-16 pt-10 border-t border-stone-100 text-center">
                        <span className="text-6xl font-black text-stone-800">{totalDaysWithTasks}</span>
                        <p className="text-stone-500 font-bold mt-3 tracking-widest uppercase text-sm">Total Days Tracked</p>
                      </div>
                    </div>
                 )}
              </div>
            ...
            : activeTab === "Sticky Notes" ? (
              <div className="max-w-5xl w-full bg-[#fce5e8] p-12 rounded-[3rem] shadow-sm border border-white flex flex-col h-[80vh]">
                <h2 className="text-3xl font-bold mb-8 text-stone-800">Global Scratchpad</h2>
                <textarea 
                  className="flex-1 bg-transparent border-none outline-none resize-none text-stone-700 text-xl leading-relaxed placeholder-stone-400/70 custom-scrollbar"
            ...
            : (
              <div className="flex flex-1 items-center justify-center text-stone-400 border-2 border-dashed border-stone-200 rounded-[3rem]">
